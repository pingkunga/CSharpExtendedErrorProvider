...
public class ErrorProviderExtended : System.Windows.Forms.ErrorProvider
{
    ...
    public bool CheckAndShowSummaryErrorMessage()
    {
        ...
        for (i = 0; i <= (Controls.Count - 1); i++)
        {
            if (Controls[i].Validate)
            {
                ...
                {
                    ClearErrorMessages(Controls[i].ControlObj);
                }
            }
            else
            {
    ...
    private void ClearErrorMessages(object ControlObj)
    {
...
public class Helper
{
    public static bool IsInheritfromControl(object pControlObj)
    {
        return pControlObj is System.Windows.Forms.Control;
    }
...
public class ValidationControlCollection : System.Collections.CollectionBase
{
    ...
    int GetIndex(string ControlName)
    {
        ...
        for (i = 0; i <= (Count - 1); i++)
        {
            if (Helper.IsInheritfromControl(this[i].ControlObj) 
            && (((System.Windows.Forms.Control)this[i].ControlObj).Name.ToUpper() == ControlName.ToUpper()))
            {
                return i;
            }
